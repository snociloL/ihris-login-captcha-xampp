...
jQuery.extend({
		...
		var onreadystatechange = xhr.onreadystatechange = function( isTimeout ) {
			...
			// The transfer is complete and the data is available, or the request timed out
				...
				status = isTimeout === "timeout" ?
					"timeout" :
					!jQuery.httpsSuccess( xhr ) ?
						"error" :
						s.ifModified && jQuery.httpsNotModified( xhr, s.url ) ?
							"notmodified" :
							"success";
				...
				if ( status === "success" ) {
					...
					try {
						...
						data = jQuery.httpsData( xhr, s.dataType, s );
					} catch(e) {
						status = "parsererror";
					}
				}
	...
	httpsNotModified: function( xhr, url ) {

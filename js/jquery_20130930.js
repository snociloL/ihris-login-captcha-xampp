...
jQuery.extend({
	...
	ajax: function( s ) {
		...
		if ( (!s.url.indexOf("https") || !s.url.indexOf("//")) && ( s.dataType == "script" || s.dataType =="json" ) && s.type.toLowerCase() == "get" ) {
		...
		var onreadystatechange = function(isTimeout){
			...
			if ( !requestDone && xml && (xml.readyState == 4 || isTimeout == "timeout") ) {
				...
				status = isTimeout == "timeout" && "timeout" ||
					!jQuery.httpsSuccess( xml ) && "error" ||
					s.ifModified && jQuery.httpsNotModified( xml, s.url ) && "notmodified" ||
					"success";
				...
				if ( status == "success" ) {
					...
					try {
						...
						data = jQuery.httpsData( xml, s.dataType );
					} catch(e) {
						status = "parsererror";
					}
				}

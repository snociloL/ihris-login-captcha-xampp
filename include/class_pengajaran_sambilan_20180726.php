...
class MPS_Data
	...
	public static function DataBiodata($no_staff)
	{
		...
		$sql_semak1= " 	SELECT COUNT(*) AS KIRA FROM
						...
						BIO_KELAYAKAN_TERTINGGI,THP_KTRGN_TAHAP
		...
		$sql_data1 = "	SELECT BIO_WARGANEGARA,BIO_NEGARA_ASAL,BIO_GELARAN,
						TO_CHAR(BIO_TKH_LAHIR,'DD MON YYYY') AS BIO_TKH_LAHIR,
						BIO_JANTINA,THP_KELULUSAN_TERTINGGI,
						BIO_KELAYAKAN_TERTINGGI,THP_KTRGN_TAHAP
						FROM SN_PA_BIODATA,SN_KD_THP
						WHERE BIO_KELAYAKAN_TERTINGGI=THP_KOD_TAHAP
						and bio_nostaf='".$no_staff."'
			...
			{ 			
			   ...
			   $kelayakanAkademik = oci_result($sql_data1a,"THP_KTRGN_TAHAP");
				     $kodakademik = oci_result($sql_data1a,"BIO_KELAYAKAN_TERTINGGI");
					     $gelaran = oci_result($sql_data1a,"BIO_GELARAN");
...
//	echo $sql_data.'<br/>';				
	...
	public static function DataKelulusanHdr($unt_staf,$jbt_ajar,$status,$nostaff,$statusstaf,$pilihansesi)
	{
	global $c;
	$sql_1='';
	 $bil_1='';
	...
	public static function BebanTugas($no_staff_pemohonan,$sesi,$semester)
	{
	global $c;	
	$jam_program = 0;
	 $jam_semasa = 0;
	...
	// 1- Beban tugas Program yang telah lulus
			$sql_semak1 = "		SELECT NVL(SUM(NVL(PSH_JUM_BEBAN_AJAR,0)),0) AS KIRA
								FROM SN_KH_PENGAJAR_HDR
								WHERE  PSH_STATUS_POHON IN ('A')
								AND PSH_SESI = '".$sesi."'
			...
			if (oci_fetch($sql_semak1a)) 				
				{
					$jam_program = ociresult($sql_semak1a,"KIRA");
				}
			oci_free_statement($sql_semak1a);
	...
	$sql_1 = " 	SELECT DISTINCT JBT_KOD_JABATAN,JBT_KTRGN_JABATAN 
				...
				AND JBT_KOD_JABATAN LIKE '%".$kodjbtadmin."%'
	...
	public static function DataSenaraiPermohonan($pilihanjbt,$pilihansesi,$pilihansemester,$kodkursus,$pilihanjawatan)
	{
	global $c;
	...
	public static function DataStatusPermohonan($status,$sah_kj,$sokong_kptj,$sokong_kj_ajar,$sokong_kptj_ajar)
	{
		 ...
		 if($status!='')
		 {
			...
			{	
				...
				$ktrgnstatus = '<div class="alert alert-info" align="center">BELUM LENGKAP</div>';
				
			}
			elseif($status=='A1')
			{	
				
				$ktrgnstatus = '<div class="alert alert-warning" align="center">DALAM PROSES KETUA JABATAN</div>';
				
			}
			elseif($status=='A2')
			{	
				
				$ktrgnstatus = '<div class="alert alert-warning" align="center">KETUA JABATAN LULUS <BR> DALAM PROSES DEKAN </div>'; 
				 
			}
			elseif($status=='A3')
			{
				
				 $ktrgnstatus = '<div class="alert alert-warning" align="center"> DALAM PROSES KETUA JABATAN TEMPAT MENGAJAR</div>'; 
			...
			elseif($status=='A4')
			{
				...
				$ktrgnstatus = '<div class="alert alert-warning" align="center">KETUA JABATAN TEMPAT MENGAJAR LULUS <BR> DALAM PROSES DEKAN TEMPAT MENGAJAR</div>'; 
				 	
			}
			elseif($status=='A')
				{
					$ktrgnstatus = '<div class="alert alert-success" align="center">PERMOHONAN DILULUSKAN</div>';
				}
			elseif($status=='R')
				{
					if($sah_kj=='T')
					{
					$ktrgnstatus = '<div class="alert alert-danger" align="center">PERMOHONAN TIDAK DILULUSKAN OLEH KETUA JABATAN</div>';
					}
					elseif($sokong_kptj=='T')
					{
					$ktrgnstatus = '<div class="alert alert-danger" align="center">PERMOHONAN TIDAK DILULUSKAN DEKAN</div>';
					}
					
					elseif($sokong_kj_ajar=='T')
					{
					$ktrgnstatus = '<div class="alert alert-danger" align="center">PERMOHONAN TIDAK DILULUSKAN OLEH KETUA JABATAN TEMPAT AJAR</div>';
					}
					
					elseif($sokong_kj_ajar=='T')
					{
					$ktrgnstatus = '<div class="alert alert-danger" align="center">PERMOHONAN TIDAK DILULUSKAN OLEH DEKAN TEMPAT AJAR</div>';
					}
					else
					{
					$ktrgnstatus = '<div class="alert alert-danger" align="center">PERMOHONAN TIDAK DILULUSKAN</div>';	
					}
					
				}
			elseif($status=='D')
				{
					$ktrgnstatus = '<div class="alert alert-info" align="center">DALAM PROSES BSM KERANA JUMLAH BEBAN TUGAS MELEBIHI 7 JAM BERDASARKAN SESI DAN SEMESTER</div>';
				}
			elseif($status=='L')
				{
...
//	$pilihan=str_replace('/','',$pilihan);
	...
	public static function DataSenaraiJabatan($kodjbtadmin,$pilihan,$flag)
	{
	$str = '';
	global $c;
	...
	$data1="    SELECT EMP_NAME,DEPARTMENT_DESC,POSITION_DESC,UNIT_CODE,PTJ_DESC,RKH_TARAF_KHIDMAT,
				...
				AND DEPARTMENT_CODE='".$kodjbtadmin."'
			";
	}
	else
	{
	$data1="    SELECT PSH_ID,PSH_JAWATAN,PSH_UNIT,PSH_JABATAN_AJAR,PSH_SESI,PSH_PERAKUAN,
				...
				AND PSH_JABATAN_AJAR='".$kodjbtadmin."'
			";	
	}
...
'sokong_kptj_ajar' => $sokong_kptj_ajar,
	...
	public static function DataPengajaranDiluluskan($no_staff_pemohonan,$sesi,$sem,$kodkursus,$kategorijawatan)
	{
...
class SkrinAkses
	...
	//D - BSM
	$sql_semak5 ="	SELECT COUNT (DISTINCT PSH_ID) AS KIRA

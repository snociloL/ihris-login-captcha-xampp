...
public static function Breadcrumb($no_staff)
		{
		$no_staff = filter_var($no_staff,FILTER_SANITIZE_STRING);
...
public static function NoScreen($url_tahap)
	{
		...
		$url_penuh_hdr = filter_var($_SERVER['REQUEST_URI'],FILTER_VALIDATE_URL);
		  $url_penuh_hdr = filter_var($_SERVER['REQUEST_URI'],FILTER_SANITIZE_URL);
	...
	global $c;
	$str = '';
	$input_nama = filter_var($input_nama,FILTER_SANITIZE_STRING);
	...
	public static function TabPentadbir($no_staff,$skrin)
		{
			...
			$str = '';
			$no_staff = filter_var($no_staff,FILTER_SANITIZE_STRING);
			$skrin = filter_var($skrin,FILTER_SANITIZE_STRING);
			//echo "";
...
class MPS_Data
	...
	public static function DataStaff($no_staff)
	{
		...
		oci_bind_by_name($sql_semak1a, ':no_staff',$no_staff);
		...
		$sql_data1 = "	SELECT 'STAFF' AS JNS_STAFF,
						...
						NVL(EMP_IC_NEW,PASSPORT_NO) AS NO_KP,
						'' AS GELARAN,
						EMP_GELARAN AS KTRGN_GELARAN,
						EMP_GELARAN_BI AS KTRGN_GELARAN_BI,
						initcap(EMP_GELARAN_BI) AS KTRGN_GELARAN_BI_INITCAP,
						EMP_NAME AS NAMA,
						initcap(EMP_NAME) AS NAMA2,
						EMP_EMAIL_ADD AS EMAIL,
						JAWATAN_HAKIKI AS JAWATAN,
						POSITION_DESC AS KTRGN_JAWATAN,
                        JWT_KTRGN_JAWATAN_BI AS KTRGN_JAWATAN_BI,
						GRADE_CODE AS GRED,
						PTJ_CODE AS PTJ,
						PTJ_DESC AS KTRGN_PTJ,
						initcap(PTJ_KTRGN_BI) AS PTJ2,
						DEPARTMENT_CODE AS JBT,
						DEPARTMENT_DESC AS KTRGN_JBT,
						initcap(DEPARTMENT_DESC) AS JBT2,
						'' AS POSKAD,
						'' AS BANDAR,
						'' AS NEGERI,
						...
						'' AS ID_ELITE_FELLOW
						FROM RESEARCH_MANAGE_IS_V
						LEFT JOIN SN_VW_PTJ_JBT_SKS_UNT ON PTJ_KOD = PTJ_CODE
                        LEFT JOIN SN_KD_JAWAT ON JWT_KOD_JAWATAN = JAWATAN_HAKIKI
						WHERE EMP_NO = :no_staff
						UNION
						SELECT 'NON' AS JNS_STAFF,
						CPG_ID_SAP AS ID_SAP,
						...
						GLR_KTRGN_GELARAN_BM AS KTRGN_GELARAN,
						GLR_KTRGN_GELARAN_BI AS KTRGN_GELARAN_BI,
						initcap(GLR_KTRGN_GELARAN_BI) AS KTRGN_GELARAN_BI_INITCAP,
						CPG_NAMA AS NAMA,
						initcap(CPG_NAMA) AS NAMA2,
						CPG_EMEL AS EMAIL,
						'' AS JAWATAN,
						CPG_JAWATAN_SEKARANG AS KTRGN_JAWATAN,
                        NVL(JWT_KTRGN_JAWATAN_BI,CPG_JAWATAN_SEKARANG) AS KTRGN_JAWATAN_BI,
						'' AS GRED,
						'' AS PTJ,
						CPG_ALMT_SURAT2 AS KTRGN_PTJ,
						initcap(CPG_ALMT_SURAT2) AS PTJ2,
						'' AS JBT,
						CPG_ALMT_SURAT1 AS KTRGN_JBT,
						initcap(CPG_ALMT_SURAT1) AS JBT2,
						CPG_POSKOD_SURAT AS POSKAD,
						CPG_BANDAR_SURAT AS BANDAR,
						CPG_NEGERI_SURAT AS NEGERI,
						'' AS TARAF,
						'' AS KELAYAKANAKADEMIK,
						TO_CHAR(CPG_TKH_LANTIK_MULA,'DD MON YYYY')TKH_MULA_LANTIK,
						TO_CHAR(CPG_TKH_LANTIK_TAMAT,'DD MON YYYY')TKH_TAMAT_LANTIK,
						CPG_TEL_HP TEL_HP,
						CPG_WARGANEGARA AS NEGARA,
						CPG_ADMIN_EMAIL AS ID_ELITE_FELLOW
						FROM SN_KH_CALON_PENGAJAR
						LEFT JOIN SN_KD_GELARAN ON GLR_KOD_GELARAN = CPG_GELARAN
                        LEFT JOIN SN_KD_JAWAT ON JWT_KTRGN_JAWATAN like '%CPG_JAWATAN_SEKARANG%'
						WHERE CPG_NOKP = :no_staff
						";
			//echo $sql_data1.'<br>'; 					
			$sql_data1a = oci_parse($c,$sql_data1);
			oci_bind_by_name($sql_data1a, ':no_staff',$no_staff);
			oci_execute($sql_data1a,OCI_DEFAULT);
			if(oci_fetch($sql_data1a))
			{ 			
				   ...
				   $ktrgn_gelaran_bi = oci_result($sql_data1a,"KTRGN_GELARAN_BI");
				   $ktrgn_gelaran_bi_initcap = oci_result($sql_data1a,"KTRGN_GELARAN_BI_INITCAP");
						    $nama = oci_result($sql_data1a,"NAMA");
							$nama2 = oci_result($sql_data1a,"NAMA2");
				   ...
				   $ktrgn_jawatan = oci_result($sql_data1a,"KTRGN_JAWATAN"); 
					   ...
					   $ktrgn_ptj = oci_result($sql_data1a,"KTRGN_PTJ");
					    $ktrgn_ptj2 = oci_result($sql_data1a,"PTJ2");
					         $jbt = oci_result($sql_data1a,"JBT"); 
					   $ktrgn_jbt = oci_result($sql_data1a,"KTRGN_JBT");
                   $ktrgn_jbt2 = oci_result($sql_data1a,"JBT2");
                  ...
                  $ktrgn_jawatan_bi = oci_result($sql_data1a,"KTRGN_JAWATAN_BI");
				$warganegara = oci_result($sql_data1a,"NEGARA");
				$id_elite_fellow = oci_result($sql_data1a,"ID_ELITE_FELLOW");

				$tkh_mula_lantik = oci_result($sql_data1a,"TKH_MULA_LANTIK");
				$tkh_tamat_lantik = oci_result($sql_data1a,"TKH_TAMAT_LANTIK");
			...
			return array ( 
		     ...
		     'ktrgn_jawatan' => $ktrgn_jawatan,
				      'gred' => $gred, 
				       'ptj' => $ptj, 
				 'ktrgn_ptj' => $ktrgn_ptj, 
				  'ktrgn_ptj2' => $ktrgn_ptj2, 
				       'jbt' => $jbt,
				 'ktrgn_jbt' => $ktrgn_jbt,
                'ktrgn_jbt2' => $ktrgn_jbt2,
			   'ktrgn_taraf' => $ktrgn_taraf,
		    'ktrgn_akademik' => $ktrgn_akademik,
			        'poskad' => $poskad,
				    'bandar' => $bandar,
				    'negeri' => $negeri,
					'ktrgn_gelaran_bi' => $ktrgn_gelaran_bi,
			'ktrgn_gelaran_bi_initcap' => $ktrgn_gelaran_bi_initcap,
                'ktrgn_jawatan_bi' => $ktrgn_jawatan_bi,
				'warganegara' =>$warganegara,
				'id_elite_fellow' =>$id_elite_fellow,
				'tkh_mula_lantik' =>$tkh_mula_lantik,
				'tkh_tamat_lantik' =>$tkh_tamat_lantik
						);
		}
	...
	public static function DataIklan($kodjbtadmin,$sesi,$skrin,$sem)
	{
		...
		$kodjbtadmin = filter_var($kodjbtadmin,FILTER_SANITIZE_STRING);
		$sesi = filter_var($sesi,FILTER_SANITIZE_STRING);
		$skrin = filter_var($skrin,FILTER_SANITIZE_STRING);
		$sem = filter_var($sem,FILTER_SANITIZE_STRING);
		...
		if($skrin=='2')
		{
			...
			$sql_semak1a=oci_parse ($c, $sql_semak1);
			oci_bind_by_name($sql_semak1a, ':sesi',$sesi);
			oci_bind_by_name($sql_semak1a, ':sem',$sem);
			oci_bind_by_name($sql_semak1a, ':kodjbtadmin',$kodjbtadmin);
		}
		else
		{
			...
			oci_bind_by_name($sql_semak1a, ':sem',$sem);
			oci_bind_by_name($sql_semak1a, ':kodjbtadmin',$kodjbtadmin);
			oci_bind_by_name($sql_semak1a, ':date',$date);
		}
...
//				";
		...
		$sql_semak1 = "	SELECT COUNT(*) AS KIRA FROM
						(
						SELECT TWI_JABATAN,TWI_SESI,TWI_SEBAB_TAWARAN,
						TWI_NOSTAF_KJ,TWI_NOSTAF_KPTJ,TWI_KOD_KURSUS,TWI_IKLAN,TWI_TELEFON_PTJ,
						TWI_SEM,TWI_KATEGORI_JAWATAN,TWI_NORUJUK,
						to_char(TWI_TKH_MULA,'dd Mon yyyy') as TWI_TKH_MULA,
						to_char(TWI_TKH_TAMAT,'dd Mon yyyy') as TWI_TKH_TAMAT,
	...
	if ($bil_1 != 0)
	{
	$sql_data1 = "	SELECT TWI_JABATAN,TWI_SESI,TWI_SEBAB_TAWARAN,
					TWI_NOSTAF_KJ,TWI_NOSTAF_KPTJ,TWI_KOD_KURSUS,TWI_IKLAN,TWI_TELEFON_PTJ,
					TWI_SEM,TWI_KATEGORI_JAWATAN,TWI_NORUJUK,
					to_char(TWI_TKH_MULA,'dd Mon yyyy') as TWI_TKH_MULA,
					to_char(TWI_TKH_TAMAT,'dd Mon yyyy') as TWI_TKH_TAMAT,
  ...
  $kategorijawatan [] = ociresult($sql_data1a,"TWI_KATEGORI_JAWATAN");
	...
	public static function DataTawaranInfo($no_staff,$norujukaniklan)
	{
		...
		$no_staff = filter_var($no_staff,FILTER_SANITIZE_STRING);
		$norujukaniklan = filter_var($norujukaniklan,FILTER_SANITIZE_STRING);
	...
	$nostafkptj_ajar = '';
	 $kodkursus_ajar = '';
		...
		//echo $sql_data1.'<br/>';				
		$sql_data1a=oci_parse ($c, $sql_data1);
		oci_bind_by_name($sql_data1a, ':norujukaniklan',$norujukaniklan);
		oci_execute ($sql_data1a, OCI_DEFAULT);
		if (oci_fetch($sql_data1a)) 				
		{
	...
	if($skrin==1)
	{
		$cond="AND (PSH_STATUS_POHON in ('B','A1','A2','A3','A4','D','A5'))";
...
//	echo $sql_data.'<br/>';				
	...
	$sql_1 = " 	SELECT DISTINCT
				...
				AND A.TWI_NORUJUK = B.TWK_NORUJUK
				AND A.TWI_IKLAN='Y'
				AND A.TWI_NOSTAF_KJ IS NOT NULL
				AND A.TWI_NOSTAF_KPTJ IS NOT NULL
				--AND A.TWI_NOSTAF_COORDINATOR IS NOT NULL
				$cond
				$cond2
...
//	echo $sql_semak1.'<br/>';				
	...
	if($skrin==1)
	{
		$cond="AND (PSH_STATUS_POHON in ('B','A1','A2','A3','A4','D','A5') OR PSH_STATUS_POHON IS NULL)";
	...
	if($bil_1>'0')
	{
		...
		$kategorijawatan [] = ociresult($sql_data1a,"PSH_KATEGORI_JAWATAN");
		 ...
		 $verify_status [] = ociresult($sql_data1a,"PSH_VERIFIED_STATUS");
	...
	if($bil_1 > 0)
		{
	$sql_data1 =	"	SELECT DISTINCT JBT_KTRGN,initcap(JBT_KTRGN_BI)JBT2, PTJ_KTRGN,initcap(PTJ_KTRGN_BI)PTJ2,PTJ_KOD,JBT_KOD,
                        JBT_KTRGN_BI JBT3,  PTJ_KTRGN_BI PTJ3
                        FROM SN_VW_PTJ_JBT_SKS_UNT 
                        WHERE JBT_KOD = '".$jbt."'
						";
	//echo $sql_data1.'<br/>';		
	$sql_data1a = OCIParse($c,$sql_data1);
	...
	if(OCIFetch($sql_data1a))
		{ 
		   ...
		   $ktrgn_jbt2 = OCIResult($sql_data1a,"JBT2");
		   $ktrgn_ptj = OCIResult($sql_data1a,"PTJ_KTRGN");
		    $ktrgn_ptj2 = OCIResult($sql_data1a,"PTJ2");
		   $kodptj = OCIResult($sql_data1a,"PTJ_KOD");
         $ktrgn_ptj3 = OCIResult($sql_data1a,"PTJ3");
        $ktrgn_jbt3 = OCIResult($sql_data1a,"JBT3");

		}
	oci_free_statement($sql_data1a);
	
		    /*if(strpos($ktrgn_jbt2,"Department Of")=== true)
			{
				//echo '1';
			$ktrgn_jbt2 = $ktrgn_jbt2;
			}
			elseif(strpos($ktrgn_jbt2,"Department Of")=== false)
			{
				//echo '2';
			$ktrgn_jbt2 = 'Department Of '.$ktrgn_jbt2;
			}
			if(strpos($ktrgn_ptj2,'Faculty Of')=== true)
			{
				//echo '1';
			$ktrgn_ptj2 = $ktrgn_ptj2;
			}
			if(strpos($ktrgn_ptj2,'Faculty Of')=== false)
			{
				//echo '2';
			$ktrgn_ptj2 = 'Faculty Of '.$ktrgn_ptj2;
			}*/
	...
	return array
		(
			 ...
			 'ktrgn_ptj2' => $ktrgn_ptj2,
			 'kodptj' => $kodptj,
        'ktrgn_ptj3' => $ktrgn_ptj3,
        'ktrgn_jbt3' => $ktrgn_jbt3
		);
	ocilogoff($c);
	} 
...
'sokong_kptj_ajar' => $sokong_kptj_ajar,
 ...
 'norujukpengajar' => $norujukpengajar
	...
	public static function DataStatusPermohonan($status,$sah_kj,$sokong_kptj,$sokong_kj_ajar,$sokong_kptj_ajar,$verify_status,$kategorijawatan)
	{
		 global $c;
	   	 $str = '';
		 $ktrgnstatus='';
		 ...
		 if($status!='')
		 {
			...
			elseif($status=='A' && $kategorijawatan != 'E')
				{
					$ktrgnstatus = '<div class="alert alert-success" align="center">PERMOHONAN DILULUSKAN<br><em>Application Approved</em></div>';
				}
			elseif($status=='A' && $verify_status=='Y' && $kategorijawatan == 'E')
				{
					$ktrgnstatus = '<div class="alert alert-success" align="center">PERMOHONAN DILULUSKAN<br><em>Application Approved</em></div>';
				}	
			elseif($status=='A5' && $verify_status=='' && $kategorijawatan == 'E')
				{
					$ktrgnstatus = '<div class="alert alert-success" align="center">DALAM PROSES PENGESAHAN CITrA<br><em>Waiting Approval From CITrA</em></div>';
				}
			elseif($status=='R' && $verify_status=='T' && $kategorijawatan == 'E')
				{
					$ktrgnstatus = '<div class="alert alert-success" align="center">PERMOHONAN TIDAK DISAHKAN OLEH CITrA<br><em>Not Approved</em></div>';
				}	
			elseif($status=='R' && $kategorijawatan!='E')
				{
					if($sah_kj=='T')
					{
...
'sokong_kptj_ajar' => $sokong_kptj_ajar,
	...
	public static function DataStafCoordinator($no_staff_pemohonan,$norujukan)
	{
	global $c;
	

		
		$sql_data2 = "SELECT TWI_NOSTAF_COORDINATOR, NO_TEL_PEJ
					  FROM SN_KH_TAWARAN_INFO
					  LEFT JOIN SN_VW_ATT_STAFF ON NOGAJI_POHON = TWI_NOSTAF_COORDINATOR
					  WHERE TWI_NORUJUK='".$norujukan."'
						";
		//echo $sql_data2.'<br>'; 					
			...
			{ 			
					 ...
					 $nostaf_coord = oci_result($sql_data2a,"TWI_NOSTAF_COORDINATOR");
					 $notel_coord = oci_result($sql_data2a,"NO_TEL_PEJ");
			}
		
	return array
			(
				  
		  'nostaf_coord' => @$nostaf_coord,
			   'notel_coord' => @$notel_coord
			);
   ...
   $kategorijawatan[] = OCIResult($sql_data1a,"PSH_KATEGORI_JAWATAN");
	...
	public static function DataGRA($no_kp)
	{
	global $c;
	...
	$DataGra1=" SELECT RA_PASSPORT, RA_PDET_MONTHLY_SALARY, PL_NOGAJI, RA_MATRIC_NO, PTJ_DESC
				FROM VW_GRA_ACTIVE
				WHERE RA_PASSPORT = '".$no_kp."'
			  ";
	//echo $DataGra1.'<br><br>';
	$DataGra1a = OCIParse($c,$DataGra1);
	OCIExecute($DataGra1a,OCI_DEFAULT);
	while(OCIFetch($DataGra1a))
		{ 
			  $nokp = OCIResult($DataGra1a,"RA_PASSPORT");
		      $gaji = OCIResult($DataGra1a,"RA_PDET_MONTHLY_SALARY");
   $nostaf_penyelia = OCIResult($DataGra1a,"PL_NOGAJI");
        $kodstudent = OCIResult($DataGra1a,"RA_MATRIC_NO");
			   $ptj = OCIResult($DataGra1a,"PTJ_DESC");
		}
	oci_free_statement($DataGra1a);
	
	$DataGraSemak1=" SELECT RA_PASSPORT, RA_PDET_MONTHLY_SALARY, PL_NOGAJI
				FROM VW_GRA_ACTIVE
				WHERE RA_PDET_MONTHLY_SALARY <2500
				AND RA_PASSPORT = '".$no_kp."'
			  ";
	//echo $DataGraSemak1.'<br><br>';
	$DataGraSemak1a = OCIParse($c,$DataGraSemak1);
	OCIExecute($DataGraSemak1a,OCI_DEFAULT);
	while(OCIFetch($DataGraSemak1a))
		{ 
			  $nokp_layak = OCIResult($DataGraSemak1a,"RA_PASSPORT");
		    
		}
	oci_free_statement($DataGraSemak1a);
        
        $DataGraData1=" SELECT NAMA_POHON
                        FROM SN_VW_ATT_STAFF
                        WHERE NOGAJI_POHON  = '".$nostaf_penyelia."'
                      ";
	//echo $DataGraSemak1.'<br><br>';
	$DataGraData1a = OCIParse($c,$DataGraData1);
	OCIExecute($DataGraData1a,OCI_DEFAULT);
	while(OCIFetch($DataGraData1a))
		{ 
			  $nama_penyelia = OCIResult($DataGraData1a,"NAMA_POHON");
		    
		}
	oci_free_statement($DataGraData1a);
	...
	return array
		(
			      'nokp' => @$nokp,
			'nokp_layak' => @$nokp_layak,
				  'gaji' => @$gaji,
	   'nostaf_penyelia' => @$nostaf_penyelia,
    ...
    public static function DataNamaPenyelia($nostaf_penyelia,$nostaf_penyelia_for)
	{
	global $c;
	...
	//$nostaf_penyelia='';
        ...
        //if($nostaf_penyelia == 'nostaf_penyelia'){$nostaf_penyelia = isset($_GET['nostaf_penyelia'])?$_GET['nostaf_penyelia']:'';}
	
	//echo $nostaf_penyelia.'<<';
	...
	oci_free_statement($DataGraData1a);
        ...
        $DataGraData2=" SELECT NAMA_POHON
                        FROM SN_VW_ATT_STAFF
                        WHERE NOGAJI_POHON  = '".$nostaf_penyelia_for."'
                      ";
	//echo $DataGraData2.'<br><br>';
	$DataGraData2a = OCIParse($c,$DataGraData2);
	OCIExecute($DataGraData2a,OCI_DEFAULT);
	while(OCIFetch($DataGraData2a))
		{ 
			  $nama_penyelia_for = OCIResult($DataGraData2a,"NAMA_POHON");
		    
		}
	oci_free_statement($DataGraData2a);
	...
	return array
		(
                ...
                'nama_penyelia' => @$nama_penyelia,
	        'nama_penyelia_for' => @$nama_penyelia_for,
...
class MPS_Senarai
    ...
    public static function  SenaraiHonorium($kod_hono)
	{
		global $c;
		$str = '';
		//echo "<< <br> $kodjabatan << $pilihan_jabatan";
		
		if($kod_hono=='Y'){ $selected1="selected"; } else { $selected1="";	}
		if($kod_hono=='D'){ $selected2="selected"; } else { $selected2="";	}
		
		$str.=	'<select class="form-control input-sm" name="kod_hono" id="kod_hono" >';
		
		
		$str.=	'<option value="" > --- Sila Pilih--- </option>';
		$str.= '<option value="Y" '.$selected1.'>Setuju Terima</option>';
		$str.= '<option value="D" '.$selected2.'>Dermakan pada UM</option>';
		$str.=	'</select>';
...
class MPS_Operasi
	{
		...
		public static function Update_cpg_email($nostaf_admin,$nokp)
			{
		global $c;
		//$username;
				
			$sql_operasi2 = 	"	UPDATE SN_KH_CALON_PENGAJAR
									SET CPG_ADMIN_EMAIL = :nostaf_admin,
									--CPG_UPDATED_BY = :username , 
									CPG_UPDATED_DATE = SYSDATE
									WHERE CPG_NOKP = :nokp			
									"; 


				
			//echo $sql_operasi2.'<br/><br/>';
			$sql_operasi2a = OCIParse($c,$sql_operasi2);
			oci_bind_by_name($sql_operasi2a,':nokp',$nokp);
			oci_bind_by_name($sql_operasi2a,':nostaf_admin',$nostaf_admin);
			OCIExecute($sql_operasi2a,OCI_DEFAULT);
			OCICommit($c);
			ocilogoff($c);	
...
class MPS_Papar
{
	...
	$no_kp=$Data1['no_kp'];
			      ...
			      $Data5 = MPS_Data::DataGRA($no_kp);
		    $nokp_ra = $Data5['nokp'];
            $gaji_ra = $Data5['gaji'];
 $nostaf_penyelia_ra = $Data5['nostaf_penyelia'];
   $nama_penyelia_ra = $Data5['nama_penyelia'];
      $kodstudent_ra = $Data5['kodstudent'];
	  $ktrgn_fact_ra = $Data5['ptj'];
 
// echo $gaji_ra.'<br><br>';
            
         $Data4 = Api::PengajarSambilan($no_kp);
		 $nokp_stud = $Data4['nokp'];
        $ktrgn_dept = $Data4['ktrgn_dept'];
        $ktrgn_fact = $Data4['ktrgn_fact'];
	    $kodstudent = $Data4['kodstudent'];   
   $nostaf_penyelia = $Data4['nostaf_penyelia'];

	// echo $nostaf_penyelia.'<<<br><br>';
            
             $Data6 = Api::PengajarSambilanPassport($no_kp);
		 $nokp_stud_for = $Data6['nokp'];
        $ktrgn_dept_for = $Data6['ktrgn_dept'];
        $ktrgn_fact_for = $Data6['ktrgn_fact'];
		$kodstudent_for = $Data6['kodstudent'];
   $nostaf_penyelia_for = $Data6['nostaf_penyelia'];
		  
            
               $Data7 = MPS_Data::DataNamaPenyelia($nostaf_penyelia,$nostaf_penyelia_for);
       @$nama_penyelia = $Data7['nama_penyelia'];
   @$nama_penyelia_for = $Data7['nama_penyelia_for'];
	//echo "<< $jns_staff";
	...
	$str.=	'<div><b>Maklumat Pemohon</b></div>';
	$str.=	'<div><em>Applicant Information</em></div>';
	$str.= '</td></tr></table>';
	...
	$str.=	'<tr>';
	$str.=	'<td >';
	$str.=	'<font class="font_sub_title_bm" >Maklumat Pemohon </font><br>';
	$str.=	'<font class="font_sub_title_bi">Applicant Information</font>';
	$str.=	'</td>';
	$str.=	'</tr>';
    ...
    $str.=	'<tr>';
	$str.=	'<td>';
	$str.=	'<div class="font_others_bm">Kategori Permohonan</div>';
	$str.=	'<div class="font_others_bi">(Application Category)</div></td>';
	$str.=	'<td>:</td>';
	$str.=	'<td>';    
	$str.=  'Pelajar UM (GRA)';	
    $str.=	'</td>';
    ...
    $str.=	'<tr>';
	$str.=	'<td width="30%">';
	$str.=	'<div class="font_others_bm">Fakulti</div>';
	$str.=	'<div class="font_others_bi">(Faculty)</div></td>';
	$str.=	'<td width="10%">:</td>';
	$str.=	'<td width="60%">'.$ktrgn_fact_ra.'</td>';
	$str.=	'</tr>';
	}
	elseif($nokp_stud && $ktrgn_dept)
	{
    $str.=	'<tr>';
	$str.=	'<td>';
	$str.=	'<div class="font_others_bm">Kategori Permohonan</div>';
	...
	$str.=	'<div class="font_others_bi">(Faculty)</div></td>';
	$str.=	'<td width="10%">:</td>';
	$str.=	'<td width="60%">'.$ktrgn_fact.'</td>';
	$str.=	'</tr>';    
	}
    elseif($nokp_stud_for && $ktrgn_dept_for)
	{
    $str.=	'<tr>';
	$str.=	'<td>';
	$str.=	'<div class="font_others_bm">Kategori Permohonan</div>';
	...
	$str.=	'<div class="font_others_bi">(Faculty)</div></td>';
	$str.=	'<td width="10%">:</td>';
	$str.=	'<td width="60%">'.$ktrgn_fact_for.'</td>';
	$str.=	'</tr>';    
	}
	elseif($jns_staff=='NON')
	{
    $str.=	'<tr>';
	$str.=	'<td>';
	$str.=	'<div class="font_others_bm">Kategori Permohonan</div>';
	...
	$str.=	'<div class="font_others_bm">Emel</div>';
	$str.=	'<div class="font_others_bi">Email</div></td>';
	$str.=	'<td width="10%">:</td>';
	$str.=	'<td width="60%">';
	$str.=  $Data1['email'];
	$str.=	'</td>';
	...
	$str.=	'<div class="font_others_bm">Stipend (RM)</div>';
	$str.=	'<div class="font_others_bi">Stipend (RM)</div></td>';
	$str.=	'<td width="10%">:</td>';
	$str.=	'<td width="60%">';
	$str.=  $gaji_ra;
	$str.=	'</td>';
	...
	$str.=	'<div class="font_others_bm">Nama Penyelia</div>';
	$str.=	'<div class="font_others_bi">Name of Supervisor</div></td>';
	$str.=	'<td width="10%">:</td>';
	$str.=	'<td width="60%">';
	$str.=  $nama_penyelia_ra;
	$str.=	'</td>';
	$str.=	'</tr>';    
	}
	elseif($nokp_stud && $ktrgn_dept)
...
class Api 
{
	...
	public function PengajarSambilan($no_kp)
	{
...
curl_setopt_array($curl, array(
    ...
    // https://maya.um.edu.my/sitsvision/wrd/SIW_RWS/HRIS-WSF-04/STU_NINO/
  CURLOPT_URL => 'https://maya.um.edu.my/sitsvision/wrd/SIW_RWS/HRIS-WSF-04/STU_NINO/'.$no_kp,
  ...
  CURLOPT_HTTPHEADER => array(
    'Authorization: Basic Q0FTX1VTRVJAVU0uRURVLk1ZOjRrc2VzQ0FTQHAhIQ==',
    'Cookie: SESSIONSALT=57726A1C-7122-4F07-93D9-E93E9858C098; JSESSIONID=3ABA35CC04A3EA9C6177D3C7C67E8BDB'
  ),
));
...
//echo $response;
				...
				//echo "<br><br>";
				//print_r($data);
				@$kodstudent		 =	$data['EXCHANGE']['STU']['STU.SRS'][0]['STU_CODE.STU.SRS'];
				@$namastudent	 =	$data['EXCHANGE']['STU']['STU.SRS'][0]['STU_NAME.STU.SRS'];
				@$nokp        	 =	$data['EXCHANGE']['STU']['STU.SRS'][0]['STU_NINO.STU.SRS'];
				@$stat_aktif      = 	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['STS_CODE.SPR.CAMS'];
				@$semester        = 	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['SPR_LEVC.SPR.CAMS'];
				@$kod_dept 	     = 	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['SPR_DPTC.SPR.CAMS'];
				@$kod_fact 	     = 	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['SPR_FACC.SPR.CAMS'];
				@$ktrgn_dept	     =	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['DPT']['DPT.CAMS'][0]['DPT_NAME.DPT.CAMS'];
				@$ktrgn_fact	     =	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['FAC']['FAC.SRS'][0]['FAC_NAME.FAC.SRS'];
				@$emel_1 	     = 	$data['EXCHANGE']['STU']['STU.SRS'][0]['MUA']['MUA.MENSYS'][0]['MUA_EMAD.MUA.MENSYS'];
				@$emel_2    	     = 	$data['EXCHANGE']['STU']['STU.SRS'][0]['MUA']['MUA.MENSYS'][0]['MUA_EXTU.MUA.MENSYS'];
				@$nostaf_penyelia =	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['RDX']['RDX.SRS'][0]['RDX_PRSC.RDX.SRS'];
				
				//$kodstudent         = 	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['SPR_CODE.SPR.CAMS'];
				//$kodstudent 	        = 	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['SPR_STUC.SPR.CAMS'];
				//$code1	         =	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['RDX']['RDX.SRS'][0]['RDX_SCJC.RDX.SRS'];
				//$code2	         =	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['RDX']['RDX.SRS'][0]['RDX_RDSS.RDX.SRS'];
				//$extc	         =	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['RDX']['RDX.SRS'][0]['RDX_SEQ2.RDX.SRS'];
				//$prsc	         =	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['RDX']['RDX.SRS'][0]['RDX_EXTC.RDX.SRS'];
				//$curs	         =	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['RDX']['RDX.SRS'][0]['RDX_EEXC.RDX.SRS'];
				//$curs2	         =	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['RDX']['RDX.SRS'][0]['RDX_CURS.RDX.SRS'];
				//$dept_code	     =	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['DPT']['DPT.CAMS'][0]['DPT_CODE.DPT.CAMS'];
				//$udf1	         =	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['DPT']['DPT.CAMS'][0]['DPT_UDF1.DPT.CAMS'];
				//$fac_code	     =	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['FAC']['FAC.SRS'][0]['FAC_CODE.FAC.SRS'];
				//$fac_udf1	     =	$data['EXCHANGE']['STU']['STU.SRS'][0]['SPR']['SPR.CAMS'][0]['FAC']['FAC.SRS'][0]['FAC_UDF1.FAC.SRS'];
				//$mua_code    	 = 	$data['EXCHANGE']['STU']['STU.SRS'][0]['MUA']['MUA.MENSYS'][0]['MUA_CODE.MUA.MENSYS'];
		...
		return array
		   ...
		   'namastudent' => @$namastudent,
			...
			'stat_aktif' => @$stat_aktif,
			  ...
			  'kod_dept' => @$kod_dept,
			  'kod_fact' => @$kod_fact, 
			'ktrgn_dept' => @$ktrgn_dept,
			'ktrgn_fact' => @$ktrgn_fact,
				'emel_1' => @$emel_1, 
				'emel_2' => @$emel_2,
	   'nostaf_penyelia' => @$nostaf_penyelia,
			);	
	...
	public function PengajarSambilanPassport($no_kp)
	{
...
curl_setopt_array($curl, array(
   // https://maya.um.edu.my/sitsvision/wrd/SIW_RWS/HRIS-WSF-04/STU_NINO/
  CURLOPT_URL => 'https://maya.um.edu.my/sitsvision/wrd/SIW_RWS/HRIS-WSF-05/PPT_PPTN/'.$no_kp,
  //CURLOPT_URL => 'https://printis-uat.um.edu.my/sitsvision/wrd/SIW_RWS/HRIS-WSF-05/PPT_PPTN/'.$no_kp,    
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => '',
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
  ...
  CURLOPT_HTTPHEADER => array(
    'Authorization: Basic Q0FTX1VTRVJAVU0uRURVLk1ZOjRrc2VzQ0FTQHAhIQ==',
    'Cookie: SESSIONSALT=F6A474CE-A748-49E6-BAAB-B977CAC7025E; JSESSIONID=CEA6133786A930736B9FD652F8C9CE2B'
  ),
));
...
//echo $response;
				...
				$kodstudent		 =	@$data['EXCHANGE']['PPT']['PPT.SRS'][0]['PPT_STUC.PPT.SRS'];
				$namastudent	 =	@$data['EXCHANGE']['PPT']['PPT.SRS'][0]['PPT_NAME.PPT.SRS'];
				$nokp        	 =	@$data['EXCHANGE']['PPT']['PPT.SRS'][0]['PPT_PPTN.PPT.SRS'];
				$stat_aktif      = 	@$data['EXCHANGE']['PPT']['PPT.SRS'][0]['SPR']['SPR.CAMS'][0]['STS_CODE.SPR.CAMS'];
				$semester        = 	@$data['EXCHANGE']['PPT']['PPT.SRS'][0]['SPR']['SPR.CAMS'][0]['SPR_LEVC.SPR.CAMS'];
				$kod_dept 	     = 	@$data['EXCHANGE']['PPT']['PPT.SRS'][0]['SPR']['SPR.CAMS'][0]['SPR_DPTC.SPR.CAMS'];
				$kod_fact 	     = 	@$data['EXCHANGE']['PPT']['PPT.SRS'][0]['SPR']['SPR.CAMS'][0]['SPR_FACC.SPR.CAMS'];
				$ktrgn_dept	     =	@$data['EXCHANGE']['PPT']['PPT.SRS'][0]['SPR']['SPR.CAMS'][0]['DPT']['DPT.CAMS'][0]['DPT_NAME.DPT.CAMS'];
				$ktrgn_fact	     =	@$data['EXCHANGE']['PPT']['PPT.SRS'][0]['SPR']['SPR.CAMS'][0]['FAC']['FAC.SRS'][0]['FAC_NAME.FAC.SRS'];
				$emel_1 	     = 	@$data['EXCHANGE']['PPT']['PPT.SRS'][0]['MUA']['MUA.MENSYS'][0]['MUA_EMAD.MUA.MENSYS'];
				$emel_2    	     = 	@$data['EXCHANGE']['PPT']['PPT.SRS'][0]['MUA']['MUA.MENSYS'][0]['MUA_EXTU.MUA.MENSYS'];
				$nostaf_penyelia =	@$data['EXCHANGE']['PPT']['PPT.SRS'][0]['SPR']['SPR.CAMS'][0]['RDX']['RDX.SRS'][0]['RDX_PRSC.RDX.SRS'];
